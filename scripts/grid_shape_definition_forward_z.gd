...
class_name GridShapeDefinitionForwardZ
...
func faceUpCoords(centerCoords, gridWidth):
	...
	return {
		"coords": [Vector2(cRow-1, cCol), Vector2(cRow, cCol), Vector2(cRow, cCol+1), Vector2(cRow+1, cCol+1)],
...
func faceRightCoords(centerCoords, gridWidth):
	...
	if (cCol == gridWidth-1): cCol -= 1
	return {
		"coords": [Vector2(cRow+1, cCol-1), Vector2(cRow+1, cCol), Vector2(cRow, cCol), Vector2(cRow, cCol+1)],
		"centerCoords": Vector2(cRow, cCol)
	}
